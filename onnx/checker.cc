...
void check_graph(
    ...
    const CheckerContext& ctx,
    LexicalScopeContext& parent_lex) {
  ...
  LexicalScopeContext current_scope(parent_lex);
  ...
  for (const auto& value_info : graph.input()) {
    current_scope.add_def(value_info.name());
  ...
  for (const auto& node : graph.node()) {
    ...
    for (const auto& input : node.input()) {
      ...
      if (!current_scope.valid_use(input)) {
        fail_check(
    ...
    try {
      check_node(node, ctx, current_scope);
    ...
    for (const auto& output : node.output()) {
      // check if optional output
      if (!output.empty()) {
        current_scope.add_def(output);
      }
    }
  }
}
...
void check_function(
  ...
  for (const auto& node : function.node()) {
    ...
    for (const auto& output : node.output()) {
      // add non-optional outputs to current scope
      if (!output.empty()) {
        current_scope.add_def(output);
      }
    }
